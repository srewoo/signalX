/**
 * Inline SVG icon factory — Lucide-style line icons (24x24 viewBox,
 * stroke=currentColor, stroke-width=2, round caps/joins, no fill).
 *
 * SECURITY: every entry in PATHS below is a COMPILE-TIME CONSTANT string
 * authored here — none of it is ever derived from RSS titles, LLM output, or
 * any other runtime/user data. We build the <svg> via createElementNS and set
 * the inner path markup from these trusted constants only. No dynamic value is
 * ever interpolated into the markup, so this cannot introduce XSS.
 */
...
/**
 * Build an icon as a detached SVGElement. `size` sets width/height in px;
 * the icon inherits its color from `currentColor` so callers tint it via CSS.
 */
export function icon(name: IconName, size = 16): SVGElement {
  ...
  // Trusted compile-time constant markup only — see module header.
