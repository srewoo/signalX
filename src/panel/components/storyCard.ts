/** Story cluster card with source pips + "✦ Summarize" (feed/search screens). */
...
export function storyCard(cluster: StoryCluster, opts: StoryCardOptions): HTMLElement {
  ...
  const sourcesRow = el('div', { class: 'sources-row' }, [
    ...
    el('button', { class: 'summarize-link', 'aria-label': `Summarize: ${cluster.headline}`, onClick: (e) => { e.stopPropagation(); opts.onSummarize(); } }, ['✦ Summarize']),
  ]);
